              ...
              GlobalDefinition
                functionDefaults
                  { LLVM.AST.Global.name = "f",
                    LLVM.AST.Global.parameters =
                      ( [ Parameter (AST.ptr (NamedTypeReference "pair")) "ptr_0" [],
              ...
              GlobalDefinition
                functionDefaults
                  { LLVM.AST.Global.name = "g",
                    LLVM.AST.Global.parameters =
                      ( [Parameter (NamedTypeReference "pair") "pair_0" []],
                        False
                      ),
                    LLVM.AST.Global.returnType = AST.i32,
                    LLVM.AST.Global.basicBlocks =
                  ...
                  { LLVM.AST.Global.name = "f",
                    ...
                    LLVM.AST.Global.basicBlocks =
                      [ BasicBlock
                          ...
                          ( Do
                              ( CondBr
                                  { condition = LocalReference (IntegerType {typeBits = 1}) (Name "a_0"),
                    ...
                    LLVM.AST.Global.basicBlocks =
                      [ BasicBlock
                        ...
                        BasicBlock
                          ...
                          ( Do
                              ( CondBr
                                  { condition = LocalReference (IntegerType {typeBits = 1}) (Name "b_0"),
...
example = mkModule $ execModuleBuilder emptyModuleBuilder $ mdo
  foo <- function "foo" [] AST.double $ \_ -> mdo
    ...
    blk2 <- block `named` "blk"
    do
      a <- fadd c1 c1 `named` "c"
      b <- fadd a a
      br blk3

    blk3 <- block `named` "blk"
    do
